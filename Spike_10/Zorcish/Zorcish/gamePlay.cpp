...
void gamePlay::setUpGame()
{
	...
	bool itemPickupable, itemInv, itemSharp;
		...
		while (getline(mapStream, line))
		{
			if (line == "LOCATION") {
				...
				if (itemTempLocation == "PLAYER") {
					playerOne->getInventory()->addItem(new item(itemTempId, itemTempName, itemTempDesc, itemPickupable, itemInv, itemSharp));
					//cout << playerOne->getFullDesc() << endl;
					...
					for each (location * l in mainWorld->locs)
					{
						if (l->areYou(itemTempLocation)) {
							//cout << l->getInventory()->getInvSize() << endl;
							l->getInventory()->addItem(new item(itemTempId, itemTempName, itemTempDesc, itemPickupable, itemInv, itemSharp));
							//cout << l->getInventory()->getInvSize() << endl;
